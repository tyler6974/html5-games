...
- **Chrome Dino**: [https://github.com/wayou/t-rex-runner](https://github.com/wayou/t-rex-runner)
	- Note: this code is modified to be able to run without using Google Chrome's proprietary calls, which allows other browsers to play this game (it's also from the older version). The *real* original code is from the Chrome team, which is linked above.
